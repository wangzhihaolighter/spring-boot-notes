...
因此一开始我们就把这两个设成一样，使得Tomcat在启动时就为最大化参数充分利用系统的效率，这个道理和`jdbcconnection pool`里的`minpool size`与`maxpool size`的需要设成一个数量是一样的原理。
...
比如说使用了CMS GC后经过jprofiler的观察，GC被触发次数非常多，而每次GC耗时仅为几毫秒。
