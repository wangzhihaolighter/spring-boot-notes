...
import com.example.springframework.boot.security.db.swagger.config.security.filter.SystemUsernamePasswordAuthenticationFilter;
import com.example.springframework.boot.security.db.swagger.config.security.handler.SystemAuthenticationFailureHandler;
import com.example.springframework.boot.security.db.swagger.config.security.handler.SystemAuthenticationSuccessHandler;
import com.example.springframework.boot.security.db.swagger.config.security.handler.SystemLoginUrlAuthenticationEntryPoint;
import com.example.springframework.boot.security.db.swagger.config.security.handler.SystemLogoutSuccessHandler;
import com.example.springframework.boot.security.db.swagger.config.util.MD5Util;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) {
        /*
        值得注意：自定义权限校验就不能忽略swagger这些路径了，需要自行管理
        如果忽略，不会执行SecurityContextPersistenceFilter，SecurityContextHolder.setContext(contextBeforeChainExecution)这一步不会执行
        AbstractSecurityInterceptor中beforeInvocation的SecurityContextHolder.getContext().getAuthentication() == null
        会报错：An Authentication object was not found in the SecurityContext
        */
