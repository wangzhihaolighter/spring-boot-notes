...
import com.example.springframework.boot.security.db.swagger.entity.SystemPermission;
...
public class SystemAccessDecisionManager implements AccessDecisionManager {
    ...
    private boolean matcherNoAuth(HttpServletRequest request) {
        return matchers("/", request)
                ...
                //swagger
                || matchers("/v2/api-docs", request)
                || matchers("/configuration/ui", request)
                || matchers("/swagger-resources/**", request)
                || matchers("/configuration/**", request)
