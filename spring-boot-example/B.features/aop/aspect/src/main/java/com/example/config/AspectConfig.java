...
import com.example.config.advice.TestAdvice;
import com.example.config.aspect.TestAspect;
...
public class AspectConfig {
  ...
  实现一个切面的方式：
  1.切面类上添加@Aspect，配置切入点，通过连接点切入
  2.实现相应的org.aopalliance.aop.Advice

  Spring AOP支持在切入点表达式中使用如下的切入点指示符：
      execution - 匹配方法执行的连接点，Spring最主要的切入点指示符。
  ...
  public Advisor pointcutAdvisor() {
    ...
    pointcut.setExpression("execution(public * com.example.controller..*.*(..))");
    DefaultPointcutAdvisor defaultPointcutAdvisor =
        new DefaultPointcutAdvisor(pointcut, new TestAdvice());
    defaultPointcutAdvisor.setOrder(Ordered.LOWEST_PRECEDENCE);
